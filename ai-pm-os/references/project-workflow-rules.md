...
- PU 审批后状态：`Approved`（Human Owner/Sponsor Approver 产出）
- APPLY 后状态：`Applied`
...
| 失败场景 | 升级路径 |
|---|---|
| PU 状态为 `Proposed`（未批准） | L3：请求 Human Owner 审批，禁止应用 |
...
| 存在 Approved Baseline 且无接管授权 | L3：请求 Human Owner 确认接管 |
