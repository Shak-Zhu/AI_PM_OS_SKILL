...
| **C-03** | **审批状态冲突** | 同一 PU 在 `PM_PENDING_UPDATES.md` 中状态为 `Approved`，但 Project Owner 声称"没有批准过" | `PM_GAP_ANALYSIS.md` → `GAP-CFL-###` + `PM_RAID_LOG.md` → `R-YYYY-###` | `Escalation: approval-conflict` |
...
- `recovery_required`：冲突需要 Project Owner 介入时
...
| **N-03** | **同对象多 ID** | 同一对象（如同一个需求）被分配了两个不同 ID | 进入 `Conflict: multi-id-for-same-object`；列出两个 ID 供 Project Owner 裁定 | 不得自动合并两个 ID 为一个 | `PM_GAP_ANALYSIS.md` |
...
3. 请求 Project Owner 提供 Markdown 源文件
...
| `conflict_detected` | 冲突分类完成，Project Owner 尚未裁定 | `escalated` / `resolved` |
| `recovery_required` | 冲突需要 Project Owner 介入 | `resolved` / `escalated` |
