...
## 8. CHG-011 — AI Coder 委派 Applicability Gate

<!-- SECTION:CODER_APPLICABILITY -->

本节定义 `COC-CWP-001`、`COC-RWP-002`、`COC-PQR-003`、`COC-HAR-006` 的 applicability 条件。

根据 CHG-011（AI Coder 委派边界）：

- Coder Work Package 仅在 Human Owner **明确启用** AI Coder 委派时触发。
- 未启用时，Skill 不得创建治理根目录、`Coder Work Package` 文件、`PM/QC Review` 报告或这些 COC 的任何制品。

### 8.1 Applicability Gate

以下 4 类 COC **仅在 Human Owner 显式启用委派时**可被触发：

| contract_id | 触发条件 | 未启用时行为 |
|---|---|---|
| `COC-CWP-001` | Human Owner 当前消息包含"签发工作包"、"Coder"、"Cursor 执行"等明确委派意图 | 不创建 `${GOVERNANCE_ROOT}/pm-ai-work-packages/` 目录 |
| `COC-RWP-002` | Human Owner 当前消息包含"返工"、"rework"、"重做"且指向上游 Coder WP | 不创建返工工作包文件 |
| `COC-PQR-003` | PM/QC 进行独立验收（由 Human Owner 或 PM PM 触发） | 不创建 QC 报告文件 |
| `COC-HAR-006` | Human Owner 请求最终 L1 验收 | 不修改 `PM_APPROVAL_STATUS.md` |

### 8.2 未启用时的 fail-closed 行为

当 Human Owner **未明确启用** AI Coder 委派时：

1. 不得创建 `${GOVERNANCE_ROOT}/pm-ai-work-packages/` 目录
2. 不得创建 `${GOVERNANCE_ROOT}/pm-ai-reviews/` 目录
3. 不得输出 "issued" / "accepted" / "pending PM/QC review" / "implemented"
4. 违反上述任何一项 → `Escalation: coder-delegation-not-authorized`

### 8.3 验证器要求

`validate-skill.js` 必须包含结构化语义检查，至少覆盖：

1. **正向**：当 Human Owner 显式启用委派时，COC 正常触发。
2. **负向-1**：未启用时，`GOVERNANCE_ROOT` 目录不得被创建。
3. **负向-2**：未启用时，不得输出 `implemented, pending PM/QC review` 等委派专属状态词。

验证器检查 CHG-011 gate 时，应检查：
- Human Owner 当前消息是否包含明确委派关键词（工作包、rework、Coder委派等）
- 如未包含，不得创建治理根目录或输出委派专属状态
